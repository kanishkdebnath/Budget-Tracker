...
- **Dark-first** to match the mockups: `DEFAULT_THEME_MODE = "dark"`. The `BudgetGradients` are applied via shared components: `NetBand` (navy-gradient hero with light text + `Money.formatShort` notation + optional `◎ Plan` sub-row), `GradientFab`, and `GradientBanner` (info/amber). The reference mockups live in `docs/design-system/*.html`; serve them over HTTP (`file://` is blocked in the browser tool) to compare. **Deferred design refinements:** gradient primary buttons, subtle card gradients, transaction-row "description-as-title", Report delta badges + `◎` plan column + top-bar export icon, Categories search, density mode.
