...
- **Design-review consistency pass** (bring the un-passed screens up to the `BudgetCard` + group-header conventions): ✅ **Plan** — group cards use `BudgetCard`; header is dot + name + **Kind chip** (`inferGroupKind`) + a `◎` **target subtotal** (`Money.formatShort`, live from the inputs); `"Income"` / `"Expense groups"` section labels split the list; the save bar **floats** (rounded navy `BudgetGradients.StickyBar` card with margins + shadow, content scrolls under it) and shows an `"N targets"` count beside the gradient Save button. ✅ **Recurring** — cards use `BudgetCard` (the old applied-state green card-tint is dropped; the green pill now carries that signal); section headers count (`SectionHeader(text, count)` → `"ACTIVE · 3"` / `"INACTIVE · 2"`); the applied check-pill shows the **month it ran** (`MonthUtils.monthLabelShort(lastRunMonth)` → "✓ Jun 2026"); LazyColumn gets the 88dp bottom inset so the last card clears the "New" FAB. **Remaining (from the review):** Settings (row leading-icon + chevron). Minor Plan deferrals: `◎` on the NetBand labels, filled (vs outlined) target inputs.
