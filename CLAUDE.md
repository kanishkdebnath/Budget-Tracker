...
- **Design-review consistency pass** (bring the un-passed screens up to the `BudgetCard` + group-header conventions): ✅ **Plan** — group cards use `BudgetCard`; header is dot + name + **Kind chip** (`inferGroupKind`) + a `◎` **target subtotal** (`Money.formatShort`, live from the inputs); `"Income"` / `"Expense groups"` section labels split the list; the save bar shows an `"N targets"` count beside the gradient Save button. **Remaining (from the review):** Recurring (→ `BudgetCard` cards, `"Active · N"` section counts, applied-date pill) and Settings (row leading-icon + chevron). Minor Plan deferrals: `◎` on the NetBand labels, filled (vs outlined) target inputs.
