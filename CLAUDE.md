...
- **Phase 3 ✅** — Room data layer under `data/`: 5 entities, DAOs (`Flow`), repositories (uniqueness/archive guards, atomic recurring apply + bulk target save), idempotent `DatabaseSeeder`, `PreferencesRepository` (DataStore), `AppContainer` (manual DI). `BudgetDatabase` (now **v2**; `MIGRATION_1_2` adds `Category.icon`), schema exported to `app/schemas/`.
...
- **Category icons ✅** — optional per-category vector icon (`Category.icon` registry key; DB **v2** + `MIGRATION_1_2` with best-effort seed-name backfill). Curated 61-icon set in `ui/icons/CategoryIcons.kt`, rendered via the shared `ui/components/CategoryIconChip` (tinted chip, color-dot fallback) on Categories/Log/Report/Plan + the transaction & recurring category pickers; picked via `IconPickerSheet`. Groups stay dot-only; export unchanged.
...
- **Design-review consistency pass** (bring the un-passed screens up to the `BudgetCard` + group-header conventions): ✅ **Plan** — group cards use `BudgetCard`; header is dot + name + **Kind chip** (`inferGroupKind`) + a `◎` **target subtotal** (`Money.formatShort`, live from the inputs); `"Income"` / `"Expense groups"` section labels split the list; the save bar **floats** (rounded navy `BudgetGradients.StickyBar` card with margins + shadow, content scrolls under it) and shows an `"N targets"` count beside the gradient Save button. ✅ **Recurring** — cards use `BudgetCard` (the old applied-state green card-tint is dropped; the green pill now carries that signal); section headers count (`SectionHeader(text, count)` → `"ACTIVE · 3"` / `"INACTIVE · 2"`); the applied check-pill shows the **month it ran** (`MonthUtils.monthLabelShort(lastRunMonth)` → "✓ Jun 2026"); LazyColumn gets the 88dp bottom inset so the last card clears the "New" FAB. ✅ **Settings** — section cards use `BudgetCard`; each `SettingTile` leads with a muted outline icon (`Payments`/`Palette`/`DensityMedium`/`Info`/`Lock`) and the picker rows (Currency/Theme/Density) get a trailing `KeyboardArrowRight` chevron (static Version/Privacy rows have none); hairline `TileDivider`s (inset to the text via `TileTextInset`) separate tiles within a section. **The sweep is complete — all 6 screens passed.** Minor Plan deferrals remain: `◎` on the NetBand labels, filled (vs outlined) target inputs.
